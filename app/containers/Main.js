...
class Main extends Component {
  ...
  render() {
    ...
    if (puzzle && !this.solve) {
      this.solve = puzzle.slice();
    }
    let height = 0;
    if (showRecord) {
        ...
        <Modal
          ...
          <View style={styles.modal}>
            <View
              ...
              {!showRecord && <Text style={styles.title}>name</Text>}
              {!showRecord && (
              ...
              <Touchable style={styles.button} onPress={this.onToggleRecord}>
                ...
                />
                <Text style={styles.buttonText}>weekrank</Text>
              </Touchable>
              <View style={{overflow: 'hidden', height}}>
                <Touchable style={styles.record} onPress={this.onToggleRecord}>
                  ...
                  {this.records.length > 0 ? (
                    ...
                    <Text style={styles.recordText}>norecord</Text>
                  )}
                </Touchable>
                {showRecord && (
  ...
  onErrorMove = () => {
    this.error++;
    const message = this.error > 3 ? 'fail' : 'nosolve';
  };
  ...
  onFinish = () => {
    ...
    if (this.error > 3) {
      setTimeout(() => {
        Alert.alert(
          'congrats',
          'success' + formatTime(elapsed) + '\n' + 'fail',
          [{text: 'ok'}, {text: 'newgame', onPress: this.onCreate}],
        );
      }, 2000);
      return;
    }
  ...
  onToggleOnline = async () => {
    if (!this.granted) {
      const upload = await new Promise((resolve, reject) => {
        Alert.alert('uploadrecord', 'uploadrecord', [
          {
            text: 'reject',
            onPress: () => {
              resolve(false);
      ...
      try {
        ...
        if (!score || score.get('elapsed') > this.records[0]) {
          if (!score) {
            ...
            score = AV.Object.createWithoutData('Record', score.id);
          }
          ...
          if (!result || !result.id) {
            ...
            Alert.alert('uploaderror', [{text: I18n.t('ok')}]);
            return;
          }
        }
        ...
        Alert.alert('uploaderror', e.message || 'uploaderror', [
          {text: 'uploaderror'},
        ]);
        return;
      }
  ...
  onRate = () => {
    ...
    Alert.alert('uploaderror', 'uploaderror', [
      {text: 'uploaderror'},
      {text: 'uploaderror', onPress: () => Linking.openURL(link)},
    ]);
  };
}
